...
def vector_angle(a, b):
    ...
    if -1 <= ((dot_product) / (normA * normB)) <= 1:
        theta = np.arccos(dot_product / (normA * normB))
...
class Car:
    ...
    def set_motor_value(self, count):
        ...
        # == PipeLine ==
        # 1. Heading값의 방향벡터와 목표지점의 세타값을 먼저 구해야할듯
        # 2. 세타값을 구하고 그 값을 steer값으로
        
        # == 첫번째 목표 ==         
        # 차량의 heading은 각도로 표현되었음        
        ...
        # 현재 지점과 목표 지점의 vector를 구해야함(목표 지점 - 처음 지점)
        ...
        # 첫번쨰 목표까지, heading과 목표지점까지의 vector의 사잇각을 구하기
        if self.flag == False:
            print(norm_heading_vector, norm_thr_vector)
            self.steer = vector_angle(norm_heading_vector, norm_thr_vector)
            
            if (self.x <= GOAL[2][0] and self.x >= GOAL[2][0] - 1) and (self.y <= GOAL[2][1] and self.y >= GOAL[2][1] - 1):
                time.sleep(1)
                sys.exit()

        elif self.x <= GOAL[0][0] and self.y <= GOAL[0][1] and self.flag == True:
            self.steer = -vector_angle(norm_heading_vector, norm_fr_vector)
            
            if (self.x <= GOAL[0][0] and self.x >= GOAL[0][0] - 1) and (self.y <= GOAL[0][1] and self.y >= GOAL[0][1] - 1):
                time.sleep(1)
        
        elif self.x <= GOAL[1][0] and self.y <= GOAL[1][1] and self.flag == True:
            self.steer = -vector_angle(norm_heading_vector, norm_sec_vector)
            
            if (self.x <= GOAL[1][0] and self.x >= GOAL[1][0] - 1) and (self.y <= GOAL[1][1] and self.y >= GOAL[1][1] - 1):
                self.flag = False
                time.sleep(1)
        
        print(f'현재 위치 : {self.x, self.y}, steer : {self.steer}')
